...
The provided [Multi-threaded](./multithreaded) example is from MathWorks of using multiple cores [for_loop.m](./multithreaded/for_loop.m). The example Slurm script [matlab-mp.bash](./multithreaded/matlab-mp.bash) can be modified to run a single node, multi-threaded MATLAB job.
...
MATLAB has support for running on [GPUs](./GPU). The provided MATLAB script [svd_matlab.m](./GPU/svd_matlab.m) performs a matrix decomposition using a GPU with the corresponding slurm script [matlab-gpu.bash](./GPU/matlab-gpu.bash).
