...
Provided in this repository are two example LS-DYNA Slurm jobs: [lsdyna_single_node_OpenMP.bash](./lsdyna_single_node_OpenMP.bash) is a script for running shared memory parallel (SMP) LS-DYNA, [lsdyna_single_node_MPI.bash](./lsdyna_single_node_MPI.bash) and [lsdyna_multi_node_MPI.bash](./lsdyna_multi_node_MPI.bash) are scripts with examples for message passing parallel (MPP) LS-DYNA.  We provide examples of single and double precision for both options. Please uncomment the line with the command you want to use and add a `#` to the beginning of the line with the command you don't want to use.  SMP should be run on a single node.  MPP can run on multiple cores of a single node or across multiple nodes.  

The example test is an [LS-DYNA EMAG test case](https://ansyshelp.ansys.com/public/account/secured?returnurl=/////Views/Secured/corp/v242/en/lsdyna_vm/vm-lsdyna-emag-001.html) which was written by Inaki Caldichoury.
This is one of many LS_DYNA examples that can bde downloaded from the [Ansys website](https://ansyshelp.ansys.com/public//////Views/Secured/corp/v242/en/lsdyna_vm/lsdyna_vm_input_files.html).

These scripts may need to be modified to properly work for your problem.  Please refer to the Ansys LS-DYNA [manuals](https://lsdyna.ansys.com/manuals/) for further information and options.
...
Thanks to [Texas A&M](https://hprc.tamu.edu/kb/Software/LST/ls-dyna/) for their excellent documentation.  Additional information on LS-DYNA memory specifications can be found in documentation by [MCW Research Computing](https://docs.rcc.mcw.edu/software/lsdyna/#memory-allocation), [UCONN](https://kb.uconn.edu/space/SH/26033653105/LS-Dyna+Guide#Memory-allocation), the [Ansys documentation](https://lsdyna.ansys.com/memory-for-implicit-analyses/), [Notes on memory settings](https://www.d3view.com/a-few-words-on-memory-settings-in-ls-dyna/), and [Implicit Memory notes](https://www.dynasupport.com/howtos/implicit/implicit-memory-notes).
